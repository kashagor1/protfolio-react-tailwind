...
// const RCard = ({ title, subTitle, result, des }) => {
//   return (
...
const RCard = ({ title, subtitle, result, desc }) => {
  return (
    <div className="w-full h-1/3 group flex">
      <div className="w-10 h-[6px] bg-black bg-opacity-30 bg-red mt-6 relative">
        <span className="absolute w-5 h-5 rounded-full -top-2 -left-3 flex justify-center items-center bg-black bg-opacity-70">
          <span className="w-3 h-3 rounded-full bg-bodyColor inline-flex group-hover:bg-designColor duration-300"></span>
        </span>
      </div>
      ...
      <div className="w-full bg-black bg-opacity-20 hover:bg-opacity-30 duration-300  rounded-lg px-10 flex flex-col justify-center gap-10 shadow-shadowOne">
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-2xl font-semibold text-gray-400 group-hover:text-white duration-300">
              {title}
            </h3>
            <p className="text-sm mt-2 text-gray-400 group-hover:text-white duration-300">
              {subtitle}
